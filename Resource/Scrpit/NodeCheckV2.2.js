/**
 * @Sub-Store-Page
 * XNAME 接口查询去重/重命名 2023-11-16 20:34:08
...
let cd = iar.cd ? iar.cd : 0;
let timeout = iar.timeout ? iar.timeout : 2000;
let writet = "";
let innum = 1728e5;
let loontrue = false;
let onen = false;
...
let rawtime = 1500;
const keyp = "3.s";
const EXPIRATION_KEY = "#sub-store-csr-expiration-time";
...
async function operator(e = [], targetPlatform, env) {
  ...
  function klog(...arg) {
    console.log('[XNAME] ' + subcoll + tzname + " : " + arg);
  }
        ...
        try {
          ...
          if (cachen > eins) {
            ...
            if (isLoon) {
              ...
              timedPush = zhTime(parseInt(readt, 10) - nt + parseInt(loontd, 10));
            } else if (target === "Surge" && Sue) {
              timedPush = zhTime(parseInt(readt, 10) - nt + parseInt(innum, 10));
            } else {
              timedPush = zhTime(parseInt(readt, 10) - nt + parseInt(TIMEDKEY, 10));
            }
  ...
  do {
    ...
    while (i < e.length) {
      ...
      if (!onen) {
        if (!offtz && (ein > (i * 2))) {
          if (i >= (e.length / 3) && i < (e.length * 2 / 3)) {
    ...
    if (eout > 3 && isSurge) {
      ...
      if (allsame) {
        ...
        $.notify('XNAME：点击以安装对应版本', '未使用带指定节点功能的 SubStore，或所有节点落地IP相同', '', {
          url: "https://raw.githubusercontent.com/sub-store-org/Sub-Store/master/config/Surge-ability.sgmodule",
        });
        return e;
      }
...
function getid(e) {
  let t = "ld";
  return FNV1a(`${t}-${e.server}-${e.port}`);
}
